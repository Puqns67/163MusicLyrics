...
namespace MusicLyricApp.Bean
{
	...
	public class LyricTimestamp : IComparable
	{
		...
		{
			if (input is not LyricTimestamp obj)
			{
	...
	/// 单行歌词信息
	/// </summary>
	public partial class LyricLineVo : IComparable
	{
		...
		public static List<LyricLineVo> Split(LyricLineVo main)
		{
			...
			var mc = RegexTimestamp().Matches(mainContent);
			...
			while (index < mc.Count)
			{
				...
				if (index + 1 < mc.Count)
				{
					...
					var content = mainContent[startIndex..nextTimestampIndex];
		...
		public int CompareTo(object input)
		{
			if (input is not LyricLineVo obj)
			{
				throw new MusicLyricException(ErrorMsg.SYSTEM_ERROR);
			}
	...
	public class SearchInfo
	{
		...
		public class InputSongId(string queryId, SearchSourceEnum searchSource, SearchTypeEnum searchType)
		{
			public string QueryId { get; } = queryId;

			public string SongId { get; }

			public SearchSourceEnum SearchSource { get; } = searchSource;

			public SearchTypeEnum SearchType { get; } = searchType;
