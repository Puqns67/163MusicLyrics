...
namespace MusicLyricApp.Utils
{
	public static partial class GlobalUtils
	{
		...
		private static partial Regex RegexIsAllLetterAndNumber();
		...
		public static SearchInfo.InputSongId CheckInputId(string input, SearchSourceEnum searchSource, SearchTypeEnum searchType)
		{
			...
			// QQ 音乐，数字+字母，直接通过
			if (searchSource == SearchSourceEnum.QQ_MUSIC && RegexIsAllLetterAndNumber().IsMatch(input))
			{
				return new SearchInfo.InputSongId(input, searchSource, searchType);
			}
		...
		[GeneratedRegex("^\\d+$")]
		private static partial Regex RegexIsNumber();
		...
		{
			return RegexIsNumber().IsMatch(s);
		}
		...
		public static string GetOutputName(SaveVo saveVo, string format)
		{
			...
			var songVo = saveVo.SongVo ?? throw new MusicLyricException("GetOutputName but songVo is null");
		...
		private static partial Regex RegexFillLength();
		...
		public static string ResolveCustomFunction(string content)
		{
			...
			try
			{
				foreach (Match match in RegexFillLength().Matches(content))
				{
					var raw = match.Value;
					...
					var split = raw[leftQuote..rightQuote].Split(',');
					// 三个参数
					if (split.Length != 3)
					{
						continue;
		...
		{
			ArgumentNullException.ThrowIfNull(arbitraryString);
		...
		public static Encoding GetEncoding(OutputEncodingEnum encodingEnum)
		{
			...
			{
				OutputEncodingEnum.GB_2312 => Encoding.GetEncoding("GB2312"),
				OutputEncodingEnum.GBK => Encoding.GetEncoding("GBK"),
